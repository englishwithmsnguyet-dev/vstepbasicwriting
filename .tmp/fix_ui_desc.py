...
code = code.replace("desc: 'Cấu trúc cơ bản nhất", "desc: 'Đây là cấu trúc cơ bản nhất")
code = code.replace("desc: 'Cấu trúc miêu tả", "desc: 'Đây là cấu trúc dùng để miêu tả")
code = code.replace("desc: 'Cấu trúc tối giản nhất", "desc: 'Đây là cấu trúc tối giản nhất")
code = code.replace("desc: 'Cấu trúc mở rộng của Dạng 1", "desc: 'Đây là cấu trúc mở rộng của Dạng 1")
code = code.replace("desc: 'Cấu trúc phức hợp thể hiện", "desc: 'Đây là cấu trúc phức hợp thể hiện")
code = code.replace("desc: 'Cấu trúc mang ý nghĩa", "desc: 'Đây là cấu trúc mang ý nghĩa")

# Fix the renderSingleStructure UI
...
end_pattern = r'        <div style="display: flex; flex-direction: column; gap: 16px;">\n    `;'

code = re.sub(start_pattern + r'[\s\S]*?' + end_pattern, new_html, code)

with open('script.js', 'w') as f:
    f.write(code)

print("Fixed UI and descriptions")
