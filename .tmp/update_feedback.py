...
window.generateFeedback = function(userStr, expectedAnswers) {
    ...
    // 2. Exact same words but wrong order
    if (missing.length === 0 && extra.length === 0) {
        return `Bạn viết sai trật tự từ rồi! Hãy sắp xếp lại nhé.`;
    }

    // 3. Replacement (used wrong word)
...
old_func_pattern = r'window\.checkNounsTranslation = function\(idx\) \{[\s\S]*?\n\}'

code = re.sub(old_func_pattern, new_logic, code)
