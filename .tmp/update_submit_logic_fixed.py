...
def update_submit():
    ...
    # Make sure we replace submitVerbs3 -> submitVerbs6 if it exists
    code = code.replace("window.submitVerbs3 = function()", "window.submitVerbs6 = function()")
    # Update results messages
...
window.submitVerbs4 = function() {"""
    ...
    # Use a regex or find to replace properly
    target = "window.submitVerbs4 = function() {"
    idx = code.find(target)
    if idx != -1:
        code = code[:idx] + new_logic + code[idx + len(target):]
    else:
        print("Could not find window.submitVerbs4 = function() {")
        
    with open('script.js', 'w') as f:
        f.write(code)

update_submit()
print("Success logic insertion")
